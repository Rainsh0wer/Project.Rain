    ...
    const CheckAnswers = function(arr, delay) {
        if (incr < arr.length) {
            ...
            if(arr[incr][0].includes('chọn ')) {
                ...
                select++; console.log("%c"+incr+"."+json.success+" => "+arr[incr][0], 'background: #cfe2f3; color: #000000; font-size: 12px') 
            } else if(arr[incr][0].includes('tick:')) {
                element = document.querySelectorAll('[id^="tickbox"]')[tick];
                arr[incr][0].includes('yes')?element.click():{};
                tick++; console.log("%c"+incr+"."+json.success+" => "+arr[incr][0], 'background: #ffffff; color: #000000; font-size: 12px') 
                ...
                console.log("%c"+incr+"."+json.success+" => "+arr[incr][0], 'background: #ffffff; color: #000000; font-size: 12px')
                ...
                choose++; console.log("%c"+incr+"."+json.success+" => "+arr[incr][0], 'background: #ffffff; color: #000000; font-size: 12px') 
                ...
                text++; console.log("%c"+incr+"."+json.free, 'background: yellow; color: #000000; font-size: 12px'); 
                nulos++;
            } else if(typeof arr[incr][0] === "string") {
                element = document.querySelectorAll('[id^="textbox"]')[text];
                element.textContent = arr[incr][0];
                text++; console.log("%c"+incr+"."+json.success+" => "+arr[incr][0], 'background: #cfe2f3; color: #000000; font-size: 12px') 
            } else console.log("%c"+incr+"."+json.error+" => "+arr[incr][0], 'background: #5e0000; color: #000000; font-size: 12px') 
            incr++;
            CheckAnswers(arr, delay);
            }, delay)
        } else {
            console.log("%c"+json.completed+(nulos>0?" "+json.none.replace('<number>', nulos):""), 'background: #005e06; color: #000000; font-size: 16px');
...
fetch("https://raw.githubusercontent.com/Rainsh0wer/Project.Rain/main/languages/languages.json").then(a=>a.json().then(b=>{SimpleLiveworksheetHack(b)}));
