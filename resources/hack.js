    ...
    const CheckAnswers = function(arr, delay) {
        if (incr < arr.length) {
            ...
            if(arr[incr][0].includes('select:')) {
                ...
                arr[incr][0].includes('yes')?element.click():{};
                tick++; console.log("%c"+incr+"."+json.success+" => "+arr[incr][0], 'background: #ffffff; color: white; font-size: 12px') 
                ...
                console.log("%c"+incr+"."+json.success+" => "+arr[incr][0], 'background: #ffffff; color: white; font-size: 12px')
                ...
                choose++; console.log("%c"+incr+"."+json.success+" => "+arr[incr][0], 'background: #ffffff; color: white; font-size: 12px') 
                ...
                text++; console.log("%c"+incr+"."+json.success+" => "+arr[incr][0], 'background: #ffffff; color: white; font-size: 12px') 
            } else console.log("%c"+incr+"."+json.error+" => "+arr[incr][0], 'background: #5e0000; color: white; font-size: 12px') 
            incr++;
            CheckAnswers(arr, delay);
            }, delay)
        } else {
            console.log("%c"+json.completed+(nulos>0?" "+json.none.replace('<number>', nulos):""), 'background: #cfe2f3; color: white; font-size: 16px');
